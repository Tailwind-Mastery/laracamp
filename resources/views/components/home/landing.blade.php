<section class="flex flex-col xl:flex-row lg:p-10 xl:px-0 bg-gradient-to-b from-black/20 to-white xl:gap-10 items-center">
    
    <div class="flex flex-col gap-10 lg:gap-12 md:w-4/5 items-center px-5 md:px-10 py-20 md:py-32">

        <h1 class="text-4xl md:text-5xl lg:text-6xl text-center xl:text-left font-semibold md:font-bold tracking-tight">
            PHP Laravel, Intertia, Vue, Nuxt, React, Next and Tailwind CSS
        </h1>

        <p class="text-xl lg:text-3xl text-center xl:text-left">
            Build your Application with the most Popular, Highly Scalable and Performant Frameworks
        </p>

        <div class="flex gap-5 items-center lg:text-lg">

            <a href="{{route('home')}}" class="bg-black text-white font-medium px-4 py-2 rounded">
                Get Started
            </a>

            <a href="{{route('home')}}" class="font-medium">
                Learn more
            </a>

        </div>

    </div>

    <img src="{{asset('storage/images/web/landing-fire.jpg')}}" alt="Landing Photo" class="w-full hidden lg:block lg:h-[50rem] xl:w-2/5 object-cover rounded-lg xl:rounded-tr-none xl:rounded-br-none shadow-xl">

</section>